...
public class ComputerGame : IGame
{
    ...
    private readonly ILogger<ComputerGame> _logger;
    ...
    public ComputerGame(IServiceProvider services, ILogger<ComputerGame> logger, IHubContext<DraughtsHub> hub, IMapper mapper, Random random)
    {
        _services = services;
        _logger = logger;
        _hub = hub;
    ...
    public async Task<bool> OnJoinAsync(string connectionId)
    {
        ...
        _logger.LogInformation("Computer game {Code} started with engine {Engine} and thinking time {ThinkingTime}ms",
            Code, Options.Engine.ToString(), Options.EngineThinkingTime.TotalMilliseconds);
    ...
    public async Task OnTakeMoveAsync(string connectionId, Coords origin, Coords destination)
    {
        ...
        if (Board.Winner != -1)
        {
            ...
            _logger.LogInformation("Computer game {Code} won by {HumanOrComputer} player", Code, Board.Winner == _humanPlayerSide ? "Human" : "Computer");
        }
        await Clients.SendAsync("GAME_UPDATED", GameModel);
    ...
    public async Task OnLeaveAsync(string connectionId)
    {
        ...
        _logger.LogInformation("Computer game {Code} cancelled because the player left", Code);
